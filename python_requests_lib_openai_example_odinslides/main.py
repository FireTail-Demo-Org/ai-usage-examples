...
from .utils import extract_json_from_string, format_error
...
def get_LLM_summarization(word_content, logger):
    ...
    if api_key is None:
        print(
            format_error(
                "API key not found. Please make sure the 'ODIN_SLIDES_LLM_API_KEY' environment variable is set to the API key for your desired LLM. (In this version of odinSlides only OpenAI APIs are supported. This will be extended to also include other LLMs in future updates.)"
            )
        )
        raise ValueError(
            "API key not found. Please make sure the 'ODIN_SLIDES_LLM_API_KEY' environment variable is set."
        )
    ...
    data = {
        ...
        "messages": [
            ...
            {
                "role": "system",
                "content": "User will ask you to shorten the Input Article. Make sure that the shortened version captures all the key points. \n  Response format: Keep the format of the Input Article. \n Output only the shortened article.",
            },
...
def get_chat_response(word_content, slideDeck, prompt, logger):
    ...
    data = {
        ...
        "messages": [
            {
                "role": "system",
                "content": "User will ask you to create or update text content for some slides"
                + (" based on the aforementioned Input Article" if word_content else "")
                + '. The response format should be a valid json format structured as this: [{"slide_number": <Float>, "title": "<String>", "content": "<String>", "narration": "<String>"},{"slide_number": <Float>, "title": "<String>", "content": "<String>", "narration": "<String>"}] \n content field in the response comprehensive enough as it is the main text of each slide. \n For content use a mix of bullet points and text when applicable. \n If you are modifying an existing slide leave the slide number unchanged but if you are adding slides to the existing slides, use decimal digits for the slide number. for example to add a slide after slide 2, use slide number 2.1, 2.2, ... \n If user asks to remove a slide, set its slide number to negative of its current value because slides with negative slide number will be excluded from presentation. \n The existing slides are as follows: '
                + json.dumps(slideDeck),
            },
            {
                "role": "system",
                "content": "For each slide the content field is the main body of the slide while the narration field is just an example transcript of the presentation of the content field. \n Never mention the slide number in the transcript.",
            },
            {
                "role": "system",
                "content": "For each slide, the content field should be the default field to modify if modification is demanded by the user for the slide, not the narration field. ",
            },
            {
                "role": "system",
                "content": "For each slide, the narration field should only be populated if explicitely asked in user prompt, otherwise should be left empty. ",
            },
            {
                "role": "system",
                "content": "Response should be valid json in the format described earlier. slide_number, title,and content are mandatory keys.",
            },
    ...
    if word_content:
        data["messages"].insert(0, {"role": "system", "content": "Input Article: " + word_content})
